...
- [x] OTP/PasswordToggle은 preview primitive가 아닌 안정 API로 둘지, experimental로 분리할지 결정한다.
...
- [x] visual regression baseline을 만든다.
...
- [x] primitive별 acceptance test 이름을 문서 checklist에 연결한다.
  - 체크박스를 완료 처리할 때 테스트 이름과 명령을 함께 남긴다.
  - Acceptance receipt command: `cargo test -p finui-primitives --lib --quiet`

| Primitive | Checklist coverage | Acceptance test names |
| --- | --- | --- |
| Dialog | Trigger/Portal/Overlay/Content/Title/Description/Close contract, announce semantics, force mount | `dialog_root_output_preserves_open_default_and_modal_contract`, `dialog_announce_output_requires_title_and_allows_description_omission`, `dialog_portal_output_preserves_force_mount_and_container_contract`, `dialog_part_state_mounts_when_open_or_force_mounted` |
| AlertDialog | cancel/action focus priority, destructive semantics, forced modal | `alert_dialog_action_output_separates_focus_priority_and_destructive_semantics`, `alert_dialog_announce_output_forces_alertdialog_role`, `alert_dialog_root_output_forces_modal_contract` |
| Layer/Portal | portal routing, force-mounted hidden state, dismiss policy, transform origin | `portal_route_output_resolves_default_named_and_nested_containers`, `portal_route_output_keeps_force_mounted_closed_content_hidden_and_noninteractive`, `layer_options_preserve_dismiss_filter_contract`, `layer_animation_output_reports_progress_origin_and_collision_parts` |
| Focus/Roving Focus | focus restore, modal inert, RTL keyboard direction, Escape close | `focus_manager_targets_dialog_initial_focus_and_restores_trigger_on_escape`, `modal_inert_output_blocks_dialog_outside_input_only_when_modal`, `horizontal_roving_focus_uses_rtl_arrow_direction`, `roving_focus_escape_requests_close_without_moving_focus` |
| DropdownMenu | item parts, checkbox/radio state, submenu, typeahead, roving focus, portal/content | `dropdown_menu_group_arrow_and_shortcut_outputs_complete_part_contract`, `dropdown_sub_trigger_and_content_outputs_preserve_radix_submenu_parts`, `dropdown_submenu_keyboard_action_enters_and_returns_by_direction`, `dropdown_menu_roving_focus_output_skips_disabled_items_and_wraps`, `dropdown_menu_content_output_preserves_state_side_align_and_mount` |
| ContextMenu | pointer origin, touch long press, keyboard open, shared menu behavior | `context_menu_pointer_open_output_preserves_pointer_origin`, `context_menu_long_press_open_output_respects_threshold_and_movement`, `context_menu_keyboard_open_output_uses_trigger_center_for_context_key_or_shift_f10`, `context_menu_roving_focus_output_uses_shared_vertical_menu_contract` |
| Menubar | top-level/content navigation state machine, RTL, typeahead | `menubar_state_machine_integrates_top_level_and_open_content_navigation`, `menubar_roving_focus_output_respects_rtl_arrow_direction`, `menubar_typeahead_index_wraps_from_current_and_skips_disabled_items` |
| NavigationMenu | hover/focus/open interaction, viewport, indicator, motion | `navigation_menu_interaction_output_connects_hover_focus_open_viewport_indicator_and_motion`, `navigation_menu_viewport_output_preserves_state_orientation_and_size`, `navigation_menu_indicator_output_preserves_visible_state` |
| Select | trigger/value/icon/viewport/scroll/item indicator/group, keyboard/typeahead | `select_keyboard_output_focuses_selected_item_when_opening`, `select_keyboard_output_commits_enter_and_cancels_escape`, `select_part_outputs_cover_value_icon_viewport_scroll_indicator_and_group`, `select_typeahead_match_wraps_from_current_and_skips_disabled_items` |
| Popover/HoverCard/Tooltip | anchor/trigger/close/arrow/focus hook/delay/hoverable content | `popover_modal_policy_output_traps_focus_only_for_open_modal_root`, `popover_focus_hook_output_respects_prevent_default_for_auto_focus_and_dismiss`, `hover_card_delay_output_opens_on_focus_and_closes_on_blur`, `tooltip_delay_output_respects_hoverable_content_grace_and_disable_flag` |
| Form controls | checkbox/radio/switch/slider/form label/message/validation preview | `form_field_label_and_message_outputs_preserve_validity_contract`, `form_validation_preview_output_matches_client_server_and_async_states`, `radio_group_root_output_preserves_item_count_contract`, `slider_root_options_preserve_radix_contract`, `switch_root_options_preserve_form_contract` |
| OTP/PasswordToggle | stable API decision, input display, hidden form value, disabled/read-only behavior | `otp_field_is_a_stable_public_api_not_preview`, `otp_hidden_input_output_carries_form_value_contract`, `password_toggle_is_a_stable_public_api_not_preview`, `password_toggle_apply_visible_respects_disabled_readonly_and_noop_state` |
| ScrollArea | native scroll, thumb drag, visibility delay, corner behavior | `scroll_area_native_scroll_output_preserves_egui_scroll_contract`, `scroll_thumb_drag_maps_track_delta_to_clamped_scroll_offset`, `scroll_area_visibility_output_respects_type_delay_and_force_mount`, `scroll_area_corner_output_mounts_only_when_both_axes_are_visible` |
| Toast | provider/store, announce queue, hotkey/focus, swipe, action alt text | `toast_provider_output_exposes_radix_contract`, `toast_announce_queue_prioritizes_foreground_and_preserves_accessible_actions`, `toast_viewport_hotkey_output_requests_focus_only_for_matching_chord`, `toast_root_output_exposes_state_swipe_and_accessibility_contract` |
...
- [x] 대표 primitive 상태가 visual snapshot으로 검증된다.

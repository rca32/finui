//! Preview external-texture surface with caller-owned transform state.

use egui::{
    Align2, Color32, FontId, Pos2, Rect, Sense, Stroke, StrokeKind, TextureId, Ui, Vec2, pos2, vec2,
};
...
pub fn media_surface_geometry(
    ...
    let handle = |center: Pos2| Rect::from_center_size(center, vec2(HANDLE_POINTS, HANDLE_POINTS));
    MediaSurfaceGeometry {
        bounds,
        fitted_rect,
        transformed_rect,
